...
```
- PostCSS Sofrting : 추후 설정 (css 코드 순서정렬 도구)
- Auto Colse Tag
- Auto Rename Tag
...
- 설정 버튼(VSCode의 모서리 왼쪽 아이콘 ) 클릭 > Settings 메뉴 선택
  ![Image](https://github.com/user-attachments/assets/2e53a323-1430-46a9-80bd-4039ec546855)

- 검색 내용으로 `format`으로 작성
- `Editor:Default Formatter` 설정(Prettier 선택)
- `Editor:Format On Save` 설정
  ![Image](https://github.com/user-attachments/assets/313ffcb3-d79a-402f-aee7-3a4d4f28ac07)
- VSCode 재실행 추천합니다.
...
- atis 폴더 : `백엔드 데이터 연동` 관련 파일들(파일명.json, 데이터 요청 등)
...
- div 태그에 역할에 맞는 이름을 `class로 지정` 함.
...
- 웹브라우저 탭 생성(새탭 페이지) : `<a href="주소" target="_blank">글자/그림</a>`V
...
- 추천 UI 라이브러리
  ...
  - NUI : https://mui.com/material-ui/all-components

- action 과 method 이해

  - `<form action="백엔드주소"></form>`
  - `<form action="백엔드주소" method="get"></form>`
    - 데이터공개됨 : `http://127.0.0.1:5500/good.html?id=iu&pass=1234`
...
- 인라인 css (추후 React 에서는 인라인 방식을 많이 활용함 : css 우선순의 적용)
...
- block, inlinen inline-block, none, flex ...
...
- 한개의 글자처럼 처리됩니다.
- width, heightm maring, padding 을 적용 못 함.
...
- block 이면서 inline 이면서, 가로정렬, 세로정렬, 여백조절 가능
- 가로로 레이아웃 배치시 최적화
...
- 큰 사이즈 부터 작은 사이즈로 max-width를 추천합니다.
- 공백을 주의해야 합니다.
...
- 경로(`>`) 선택자
- css 문제해결 : F12 개발자 도구에서 확인(아리에서 윗방향으로 해결)
...
- slidesPerGroup : 한 번 이동 시 몇 개의 슬라이드 이동 갯수
...
![Image](https://github.com/user-attachments/assets/6f6cd9ed-a47a-4078-ab68-916db3482faa)

- `Settings 메뉴` 선택

![Image](https://github.com/user-attachments/assets/ea0bb52b-59bd-4980-9913-471bbbf3517e)
...
- 구글, 네이버 : 검색어를 입력 시 관련 순위로 목록 제시함.
